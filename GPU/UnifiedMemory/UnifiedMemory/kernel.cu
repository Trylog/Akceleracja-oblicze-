...
void distinguish(int m, int n, int k, T* A, T* B) {
	...
	if (std::is_same<T, double>::value || std::is_same<T, float>::value) {
		...
		cudaMallocManaged(&C, closestM * closestK * sizeof(U));
		cudaMallocManaged(&D, closestM * closestK * sizeof(U));
	}
	...
	if constexpr (std::is_same<U, float>::value || std::is_same<U, double>::value) {
		...
		//start asynchronously (in-the-background) transfer data ahead of time
		cudaMemPrefetchAsync(A, closestM * closestN * sizeof(T), id);
		cudaMemPrefetchAsync(B, closestN * closestK * sizeof(T), id);
		...
		cudaMemPrefetchAsync(C, closestM * closestK * sizeof(U), cudaCpuDeviceId);
